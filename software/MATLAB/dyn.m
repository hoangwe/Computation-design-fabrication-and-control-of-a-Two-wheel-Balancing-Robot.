...
tau1 = ((2*m + M)*R^2 + 2*Jw + 2*n^2*Jm)*the_2dot + (M*L*R*cos(psi) - 2*n^2*Jm)*psi_2dot - M*L*R*(psi_dot)^2*sin(psi);
tau2 = (M*L*R*cos(psi) - 2*n^2*Jm)*the_2dot + (M*L^2 + J_psi + 2*n^2*Jm)*psi_2dot - (M*g*L*sin(psi) + M*L^2*(phi_dot)^2*sin(psi)*cos(psi));
tau3 = (0.5*m*W^2 + J_phi + 0.5*(W/R)^2*(Jw + n^2*Jm) + M*L^2*sin(psi)^2)*phi_2dot + 2*M*L^2*psi_dot*phi_dot*sin(psi)*cos(psi);
