...
int leftdir = 30;   //Control direction left motor//24
int leftdir2 = 28;  //Control direction left motor//26
...
float sw , sphi, lamda1 = 0.4, lamda2 = 0.3;
float thetadot, psidot, phidot, epsi, etheta, epsidot, ephi, ephidot, ethetadot, ethetaold;
float thetaold = 0, psiold = 0, phiold = 0 , ephiold = 0, epsiold = 0, theta_p = 0;
...
void loop() {
  if (flag_Timer1 == true) {
    ...
    psi = (mpudata  - 10) * ToRad;                      //Read psi value and convert to Rad
    ...
    //
    getlqr(sw, epsi, epsidot, sphi);
    motorcontrol(PWML, PWMR, (mpudata - 10), falldown);
    // Send data to serial
    Data = (String)(psi * ToDeg) + ',' + (String)(theta * ToDeg) + ',' + (String)(phi * ToDeg);
    Serial.println(Data);
  }
}
...
float Fuzzy_sw(float x_vao)
{
  float a = -0.9, b = -0.6, c = -0.3, d = 0, e = 0.3, f = 0.6, g = 0.9;
  float y_NB, y_NM, y_NS, y_ZO, y_PS, y_PM, y_PB, U_F_ra_1;
  //hàm NB
  if (x_vao <= a)
    y_NB = 1;
  else if (x_vao > a && x_vao <= b )
    y_NB = x_vao / (a - b) - b / (a - b);
  else
    y_NB = 0;

  // hàm NM
  if (x_vao > a && x_vao <= b )
    y_NM = x_vao / (b - a) - a / (b - a);
  else if (x_vao > b && x_vao <= c )
    y_NM = x_vao / (c - b) - c / (c - b);
  else
    y_NM = 0;

  // hàm NS
  if (x_vao > b && x_vao <= c )
    y_NS = x_vao / (c - b) - b / (c - b);
  else if (x_vao > c && x_vao <= d )
    y_NS = x_vao / (d - c) - d / (d - c);
  else
    y_NS = 0;

  // hàm ZO
  if (x_vao > c && x_vao <= d )
    y_ZO = x_vao / (d - c) - c / (d - c);
  else if (x_vao > d && x_vao <= e )
    y_ZO = x_vao / (e - d) - e / (e - d);
  else
    y_ZO = 0;

  // hàm PS
  if (x_vao > d && x_vao <= e )
    y_PS = x_vao / (e - d) - d / (e - d);
  else if (x_vao > e && x_vao <= f )
    y_PS = x_vao / (f - e) - f / (f - e);
  else
    y_PS = 0;

  // hàm PS
  if (x_vao > e && x_vao <= f )
    y_PM = x_vao / (f - e) - e / (f - e);
  else if (x_vao > f && x_vao <= g )
    y_PM = x_vao / (g - f) - g / (g - f);
  else
    y_PM = 0;
  ...
  if ( x_vao <= f )
    ...
    y_PB = x_vao / (g - f) - f / (g - f);
  else
    y_PB = 1;

  U_F_ra_1 = (-15 * y_NB - 8.5 * y_NM - 5.4 * y_NS + 0 * y_ZO + 5.4 * y_PS + 8.5 * y_PM + 15 * y_PB) / (y_NB + y_NM + y_NS + y_ZO + y_PS + y_PM + y_PB);
  return U_F_ra_1;
}
float FUZZY_H_psi(float x_vao)
{
  float aa, bb, cc, a = 29 , b = 35.5, c = 42;
  float y_NB, y_NM, y_NS, U_F_ra_1;
  if (x_vao <= a)
    y_NB = 1;
  else if (x_vao > a && x_vao <= b )
    y_NB = x_vao / (a - b) - b / (a - b);
  ...
  bb = 95;
  cc = 90;
  U_F_ra_1 = (aa * y_NB + bb * y_NM  + cc * y_NS) / (y_NB + y_NM + y_NS);
  return U_F_ra_1;
}
float FUZZY_H_psi_dt(float x_vao)
{
  float  aa, bb, cc, a = 29 , b = 35.5, c = 42;
  float y_NB, y_NM, y_NS, U_F_ra_1;
  if (x_vao <= a)
...
float FUZZY_Sphi(float x_vao)
{
  float a = -3.6, b = -2.4, c = -1.2, d = 0, e = 1.2, f = 2.4, g = 3.6;
  float y_NB, y_NM, y_NS, y_ZO, y_PS, y_PM, y_PB, U_F_ra;
  // hàm  NB
  if (x_vao <= a)
    y_NB = 1;
  else if (x_vao > a && x_vao <= b )
  ...
  U_F_ra = (-0.8 * y_NB - 0.3 * y_NM - 0.1 * y_NS + 0 * y_ZO + 0.1 * y_PS + 0.3 * y_PM + 0.8 * y_PB) / (y_NB + y_NM + y_NS + y_ZO + y_PS + y_PM + y_PB);
  return U_F_ra;
}
...
//}
//LQR function
void getlqr(float sw_, float epsi_, float epsidot_, float sphi_) {


  righvolt = epsi_ * FUZZY_H_psi( 34) + epsidot_ * FUZZY_H_psi_dt( 34) + FUZZY_Sphi( sphi_);
  leftvolt = epsi_ * FUZZY_H_psi( 34) + epsidot_ * FUZZY_H_psi_dt( 34) - FUZZY_Sphi( sphi_);
  int cap_xung = 180;
...
void update_var() {
  ...
  ephi = phi - addphi;
  etheta = theta - addtheta;

  ethetadot = (etheta - ethetaold) / dt;
  ephidot   = (ephi - ephiold) / dt;

  sw = etheta * lamda1 + ethetadot;
  sphi = ephi * lamda2 + ephidot;

  epsi = psi - Fuzzy_sw(sw);
  epsidot = (epsi - epsiold) / dt;
